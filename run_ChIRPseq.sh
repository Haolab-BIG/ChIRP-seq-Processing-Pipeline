...
process_group(){
    ...
    if [[ -n "$r2" ]]; then
        ...
        $SING_EXEC rm ${bowtieoutdir}/${base}.1.fastq.gz
        $SING_EXEC rm ${bowtieoutdir}/${base}.2.fastq.gz
    else
        # ---- SINGLE-END workflow ----
        ...
        $SING_EXEC rm ${bowtieoutdir}/${base}.fastq.gz
    fi
    ...
    $SING_EXEC rm -f "${bowtieoutdir}/${base}.sort.bam"
...
########### remove
[ -n "$FastQCdir" ]   && $SING_EXEC rm -rf "${FastQCdir}"
[ -n "$trimedfadir" ] && $SING_EXEC rm -rf "${trimedfadir}"
[ -n "$FigureDir" ]   && $SING_EXEC rm -f  "${FigureDir}"/*.peak.gz
[ -n "$outputdir" ]   && $SING_EXEC rm -rf "${outputdir}/rawdata"
